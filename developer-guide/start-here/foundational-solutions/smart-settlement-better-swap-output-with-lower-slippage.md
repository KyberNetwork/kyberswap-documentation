...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| When the pool is selected                                    | At quote time — before you submit the transaction                                                                                                                   | At execution time — on-chain, inside your transaction                                                                                                  |
| What it optimizes                                            | Finds the best route and pool split across all available liquidity sources                                                                                          | Verifies the pool selection at the moment your swap executes, and switches to a better candidate pool if one is available.                             |
| What happens if pool conditions change during execution time | You may receive whatever rate the pool offers at execution, even if it has worsened since quoting, as long as it remains above your minimum accepted return amount. | Smart Settlement detects the worsened rate and can switch to a better candidate pool at execution time, potentially improving your final output.       |
| PropAMM shows a tight quote then widens spread               | You may receive fewer tokens than quoted                                                                                                                            | Smart Settlement detects the worse rate on-chain and switches to a pool with better output                                                             |
| Your pool gets sandwich-attacked                             | You may absorb the price impact and receive fewer tokens than quoted.                                                                                               | Smart Settlement detects the impact and can switch to an unaffected pool with better output.                                                           |
| Liquidity is removed after quoting                           | You may experience higher slippage due to shallower pool depth and receive fewer tokens than quoted.                                                                | Smart Settlement detects the reduced depth and can switch to a pool with sufficient liquidity.                                                         |
| Transaction revert risk                                      | Swap reverts if slippage exceeds your tolerance                                                                                                                     | Fallback candidates reduce revert rate — if the main pool would revert, an alternative can execute instead                                             |
...
10. **Why do I see "Call Failed" warnings on my transaction hash link?**
